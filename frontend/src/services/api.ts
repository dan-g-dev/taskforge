...
// In local dev, a bare '/api/...' path works via vite.config.ts's proxy
// to localhost:4000. In production (Vercel), there is no such proxy —
// the frontend and backend are different domains — so VITE_API_URL
// (set in Vercel's environment variables) must be prepended instead.
...
export const api = {
  // Health
  checkHealth: () =>
    request<{ status: string; app: string; version: string }>("/api/health"),
  ...
  getDemoUsers: () =>
    request<{
      success: boolean;
      demoUsers: (SafeUser & { defaultPassword: string })[];
    }>("/api/auth/demo-users"),

  // Organization
  getOrganization: () =>
    request<{ success: boolean; organization: Organization; stats: any }>(
      "/api/organization",
    ),
  updateOrganization: (name: string) =>
    request<{ success: boolean; organization: Organization }>(
      "/api/organization",
      {
        method: "PUT",
  ...
  listOrgUsers: () =>
    request<{ success: boolean; users: SafeUser[] }>("/api/organization/users"),
  addOrgUser: (payload: {
    email: string;
    firstName: string;
    lastName: string;
  ...
  updateUserRole: (userId: string, role: UserRole) =>
    request<{ success: boolean; user: SafeUser }>(
  ...
  listTasks: (filters?: {
    ...
    return request<{
      success: boolean;
      tasks: (Task & { assignee?: SafeUser; creator?: SafeUser })[];
  ...
  updateTask: (
    id: string,
    payload: Partial<Task> & { dueDate?: string; assigneeId?: string },
  ) =>
  ...
  listComments: (taskId: string) =>
    request<{ success: boolean; comments: (Comment & { user?: SafeUser })[] }>(
      `/api/tasks/${taskId}/comments`,
    ),
  createComment: (taskId: string, content: string) =>
    request<{ success: boolean; comment: Comment & { user?: SafeUser } }>(
      `/api/tasks/${taskId}/comments`,
      {
        method: "POST",
  ...
  updateComment: (commentId: string, content: string) =>
    request<{ success: boolean; comment: Comment }>(
  ...
  uploadAttachment: (taskId: string, file: File) => {
    ...
    return request<{
      success: boolean;
      attachment: Attachment & { user?: SafeUser };
  ...
  markAllNotificationsRead: () =>
    request<{ success: boolean; markedCount: number }>(
  ...
  listActivity: (projectId?: string) => {
    ...
    return request<{
      success: boolean;
      activity: (ActivityLog & { user?: SafeUser })[];
    }>(`/api/activity${query}`);
  },
  getDashboard: () =>
    request<{
